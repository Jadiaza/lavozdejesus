...
const compactTextClass: Record<TextSize, string> = {
  sm: "text-[0.72rem] leading-[1.35]",
  md: "text-[0.82rem] leading-[1.4]",
  lg: "text-[0.95rem] leading-[1.42]",
};
...
export const PrayerStepCard = ({ bead, mystery, textSize, highContrast, compact = false }: Props) => {
  ...
  const activeTextClass = compact ? compactTextClass[textSize] : textClass[textSize];

  return (
    <article
      className={`${compact ? "rounded-[1.1rem] p-3 min-[390px]:p-3.5" : "rounded-[1.7rem] p-5 min-[390px]:p-6"} border shadow-[0_18px_45px_rgba(0,0,0,0.24)] ${
        highContrast
    ...
    >
      <div className={`${compact ? "mb-2" : "mb-4"} flex items-center gap-3`}>
        ...
        <p className={`${compact ? "text-[7px] tracking-[0.22em]" : "text-[9px] tracking-[0.28em]"} shrink-0 font-semibold uppercase text-gold/75`}>
          {bead.label}
        </p>
        <span className="h-px flex-1 bg-gradient-to-l from-transparent to-gold/45" aria-hidden="true" />
      </div>

      <h2 className={`text-center font-display font-semibold tracking-[-0.02em] text-foreground ${compact ? "text-[clamp(1.1rem,4.8vw,1.45rem)] leading-[1]" : "text-[clamp(1.7rem,7vw,2.25rem)] leading-[1.05]"}`}>
        {isMysteryStep && mystery ? mystery.title : prayer.title}
      </h2>

      {isMysteryStep && mystery ? (
        <div className={`${compact ? "mt-2 space-y-1.5" : "mt-5 space-y-3"} text-center`}>
          <p className={`${compact ? "text-[0.7rem]" : "text-sm"} font-display italic text-gold-bright`}>
      ...
      <div className={`mx-auto max-w-[36rem] text-center ${compact ? "mt-2 space-y-1.5" : "mt-5 space-y-4"}`}>
        {prayer.body.map((p, i) => (
