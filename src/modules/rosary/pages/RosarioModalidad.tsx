import { ArrowLeft, ChevronRight, Hand, Headphones, Sparkles } from "lucide-react";
...
export const RosarioModalidad = () => {
  ...
  return (
    <main className="relative flex h-dvh flex-col overflow-hidden bg-navy-deep text-foreground sm:block sm:h-auto sm:min-h-dvh sm:overflow-x-hidden sm:overflow-y-visible">
      <section className="relative mx-auto h-[clamp(11.5rem,24dvh,13rem)] w-full shrink-0 max-w-[1440px] overflow-hidden px-5 pb-2 pt-[max(0.6rem,env(safe-area-inset-top))] sm:min-h-[31rem] sm:px-10 sm:pb-10 lg:min-h-[36rem] lg:px-14 lg:pt-10">
        <img src={heroArt} alt="La Virgen María en oración junto a un rosario y una vela" className="absolute inset-0 h-full w-full object-contain object-top sm:object-cover sm:object-center" />
      ...
      <section className="relative z-10 mx-auto grid min-h-0 w-full max-w-[1380px] flex-1 grid-rows-3 gap-1.5 px-3 pb-[max(0.35rem,env(safe-area-inset-bottom))] sm:-mt-3 sm:block sm:space-y-6 sm:px-8 sm:pb-14 lg:-mt-8 lg:space-y-8 lg:px-14">
        {MODES.map((mode) => (
          <button key={mode.id} type="button" onClick={() => choose(mode.id)} className="group grid min-h-0 w-full grid-cols-[38%_62%] overflow-hidden rounded-[1.1rem] border border-gold/75 bg-navy text-left shadow-[0_24px_60px_-35px_rgba(0,0,0,0.95)] transition duration-300 hover:border-gold-bright sm:min-h-0 sm:grid-cols-1 sm:rounded-[1.6rem] sm:hover:-translate-y-1 lg:min-h-[21rem] lg:grid-cols-[43%_57%] lg:rounded-[2.2rem]">
            ...
            <span className="relative flex min-w-0 items-center gap-1.5 px-2 py-1.5 sm:gap-4 sm:px-10 sm:py-7 lg:gap-8 lg:px-12 lg:py-10">
              <span className="hidden shrink-0 text-gold-bright sm:block"><mode.icon className="h-12 w-12 lg:h-20 lg:w-20" strokeWidth={1.35} aria-hidden="true" /></span>
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1.5 flex max-w-[20rem] items-center gap-2 sm:mt-5 sm:gap-3" aria-hidden="true">
        ...
        <blockquote className="hidden px-4 pb-3 pt-3 text-center text-foreground/90 sm:block lg:pt-6">
          <div className="mx-auto flex items-center justify-center gap-8">
            <span className="hidden text-5xl text-gold/80 sm:block">❧</span>
            <div>
              <div className="mx-auto mb-2 text-4xl text-gold-bright">♡</div>
              <p className="font-display text-xl sm:text-2xl lg:text-3xl">El Rosario es una cadena que nos une al Cielo.</p>
              <cite className="mt-1 block text-sm not-italic text-gold/70 sm:text-lg lg:text-2xl">– San Luis María Grignion de Montfort –</cite>
            </div>
            <span className="hidden scale-x-[-1] text-5xl text-gold/80 sm:block">❧</span>
          </div>
        </blockquote>
      </section>
    </main>
