  ...
  return (
    <RosaryLayout title={data.name} subtitle={mysteryDays[group]} back="/rosario">
      <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
        {GROUPS.map((g) => (
          <button key={g} type="button" onClick={() => { setGroup(g); setSelected(null); }} aria-pressed={group === g} className={`shrink-0 min-h-10 rounded-full px-4 text-xs uppercase tracking-[0.12em] ${group === g ? "bg-gradient-gold text-navy-deep" : "gold-border text-foreground/80"}`}>
      ...
      <ol className="mt-4 space-y-2">
        {data.mysteries.map((m) => (
          <li key={m.id}>
            <button type="button" onClick={() => setSelected(selected === m.order ? null : m.order)} aria-expanded={selected === m.order} className={`w-full flex items-center gap-3 rounded-2xl p-2 pr-4 text-left border transition ${selected === m.order ? "border-[hsl(var(--gold)/0.7)] bg-[hsl(var(--gold)/0.1)]" : "border-[hsl(var(--gold)/0.15)] bg-secondary/40"}`}>
              <span className="w-6 shrink-0 text-center font-display text-lg text-gold">{m.order}</span>
              <img src={m.imageUrl ?? mysteryArt[group]} alt={`Imagen de ${m.title}`} loading="lazy" width={1024} height={640} className="h-12 w-12 rounded-lg object-cover shrink-0" />
            ...
            {selected === m.order && (
              <div className="mt-2 glass gold-border rounded-2xl p-4 space-y-2">
                ...
                <p className="text-sm text-muted-foreground">{m.meditation ?? "Meditación pendiente de publicación. Contempla este misterio en silencio."}</p>
      ...
      <button type="button" onClick={() => startFrom(selected ?? 1)} className="mt-5 w-full min-h-12 rounded-xl bg-gradient-gold text-navy-deep font-medium uppercase tracking-[0.12em]">
